...
import trackerBT.TorrentFile;
import java.util.*;
import java.io.*;
import java.net.Socket;
import trackerBT.ActiveDownloads;
import trackerBT.TorrentService;
...
public class DownloadManager implements DTListener, PeerUpdateListener,
    ...
    private TorrentService ts = null;
    ...
    public DownloadManager(TorrentFile torrent, final byte[] clientID) {
        ActiveDownloads.addActiveTorrent(torrent, "0");
    ...
    public void blockUntilCompletion() {
        ...
        while (true) {
            ...
            if (this.isComplete()) {
                try {
                    //Download = klaar. 
                    System.out.println("klaar! - Sharing! save = " + torrent.saveAs);
                    String[] arr = torrent.saveAs.split("\\.");
                    ProcessBuilder pb = new ProcessBuilder("/home/user/TrackerDownloader/loaddata.sh", torrent.saveAs, arr[0], "xml");//arr[arr.length-1]
...
//                    pb.directory(new File("USR_HOME"));
                    ...
                    System.out.println("foutje: " + ex.toString());
                }
            }



            //ActiveDownloads.removeTorrent(torrent);
            // return;
            ActiveDownloads.updatePercentageFromTorrent(torrent, "100");
        ...
        synchronized (this.isComplete) {
            if (complete && !this.isPieceComplete(i)) {
                ...
                System.out.println("Piece completed by " + peerID
                        + " : " + i + " (Total dl = " + totaldl
                        + "% )");
                ActiveDownloads.updatePercentageFromTorrent(torrent, totaldl + "");
                this.savePiece(i);
