...
import Wizard.PublishPanel;
import Wizard.PublishPanel.Task;
import jBittorrentAPI.ClientHttpRequest;
import java.util.*;
import java.io.*;
    ...
    public void blockUntilCompletion() {
        ...
        while (true) {
            ...
            if (this.isComplete()) {
                ...
                System.out.println("\r\nklaar... Press Ctrl+C to stop client");
                String trackerUrl = torrent.announceURL.replace("/announce", "/") + torrent.saveAs;
                System.out.println("request naar: " + trackerUrl);

                try {
                    ClientHttpRequest c = new ClientHttpRequest(trackerUrl);

                    InputStream inn = c.post();
                    InputStreamReader is = new InputStreamReader(inn);
                    BufferedReader bu = new BufferedReader(is);
                    String read;
                    int progress = 0;
                    while ((read = bu.readLine()) != null) {
                        progress = Math.min((int) Double.parseDouble(read), 100);
